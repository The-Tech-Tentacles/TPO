    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {state.companies.map((c) => (
          <Card
            key={c.id}
            className="cursor-pointer rounded-2xl p-5 transition hover:shadow-md"
            onClick={() => setView(c)}
          >
...
function AddCompany({
  ...
  const update = (k: keyof Company, v: string | number) => setC((p) => ({ ...p, [k]: v }));
  return (
        ...
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <Input onChange={(e) => update("industry", e.target.value)} />
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
            ...
            <div>
              <Label className="text-xs">Location / HQ</Label>
              <Input onChange={(e) => update("location", e.target.value)} />
            </div>
          </div>
          <div>
            <Label className="text-xs">Description</Label>
            <Textarea rows={3} onChange={(e) => update("description", e.target.value)} />
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <Label className="text-xs">HR Name</Label>
              <Input onChange={(e) => update("hrName", e.target.value)} />
            </div>
            <div>
              <Label className="text-xs">HR Email</Label>
              <Input type="email" onChange={(e) => update("hrEmail", e.target.value)} />
            </div>
            <div>
              <Label className="text-xs">HR Phone</Label>
              <Input onChange={(e) => update("hrPhone", e.target.value)} />
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <Label className="text-xs">Min CTC (LPA)</Label>
              <Input type="number" onChange={(e) => update("ctcMin", +e.target.value)} />
            </div>
            <div>
              <Label className="text-xs">Max CTC (LPA)</Label>
              <Input type="number" onChange={(e) => update("ctcMax", +e.target.value)} />
            </div>
          </div>
        </div>
        <DialogFooter>
          ...
          <Button onClick={() => onAdd(c as Company)} disabled={!c.name || !c.industry}>
            Save
          </Button>
        </DialogFooter>
      </DialogContent>
    ...
    <Sheet open={!!company} onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-lg">
        ...
        <div className="space-y-4 p-4">
          ...
          {company.ctcMin != null && (
            <Card className="p-3">
              <div className="text-xs text-muted-foreground">CTC Range</div>
                  ...
                  <tbody>
                    {placed.map((s) => (
                      <tr key={s.id} className="border-t">
                        ...
                        <td className="p-2">₹{s.packageLpa} LPA</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
