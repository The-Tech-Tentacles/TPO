          ...
          {selectedDateEvents.length > 0 ? (
            <div className="grid gap-4">
              {selectedDateEvents.map((event) => (
                <Card
                  key={event.id}
                  className="border-emerald-100/50 dark:border-emerald-900/20 shadow-sm hover:shadow-md transition-shadow"
                >
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between gap-4">
                      <CardTitle className="text-lg">{event.title}</CardTitle>
                      <Badge
                        variant={event.type === "Placement" ? "default" : "secondary"}
                        className={
                          event.type === "Placement" ? "bg-emerald-600 hover:bg-emerald-700" : ""
                        }
                      >
                        {event.type}
                      </Badge>
          ...
          <div className="mt-8">
            ...
            <div className="grid gap-3">
              {EVENTS.sort((a, b) => a.date.getTime() - b.date.getTime()).map((event) => (
                <div
                  key={event.id}
                  className="flex items-start gap-4 p-3 rounded-lg border border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-900/50 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/80 transition-colors"
